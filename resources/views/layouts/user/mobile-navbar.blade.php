...
<div class="responsive-navbar offcanvas offcanvas-start" data-bs-backdrop="static" tabindex="-1" id="navbarOffcanvas">
    ...
    <div class="offcanvas-body">
        <div class="accordion" id="navbarAccordion">
            @foreach ($categories as $category)
                <div class="accordion-item">
                    <button class="accordion-button" type="button" data-target="#collapse{{ $loop->index }}">
                        {{ $categories[$loop->index]->name }}
                    </button>
                    <div id="collapse{{ $loop->index }}" class="accordion-collapse collapse">
                        <div class="accordion-body">
                            <div class="accordion">
                                ...
                                @empty
                                    <div class="accordion-item">Data Kosong.</div>
                                @endforelse
                            </div>
                        </div>
                    </div>
        ...
        <div class="offcanvas-contact-info">
            <div class="accordion">
                <div class="accordion-item">
                    <a href="/about-us" class="accordion-link">
            ...
            @auth
                <div class="row">
                                    ...
                                    @role('author')
                                        <a href="{{ route('profile.author') }}">
                                            <img src="{{ asset(Auth::user()->photo ? 'storage/' . Auth::user()->photo : 'default.png') }}"
                                                alt="Image" width="55px" height="55px"
                    ...
                    <div class="col-lg-8 col-8">
                        ...
                        <div class="">
                            <p class="d-inline-block text-truncate" style="font-size: 14px;max-width: 150px;">
                                {{ auth()->user()->email }}
                            </p>
                        </div>
                    </div>
                    <div class="col-lg-1 col-1">
                        <div class="d-flex justify-content-end">
                            @role('user')
                                <a href="{{ route('profile-user.user') }}" class="btn btn-sm"
                            ...
                            @endrole
                            @role('admin')
                                <a href="{{ route('dashboard.admin') }}" class="btn btn-sm"
                                    style="background-color: #175A95">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="18" height="23"
                                        viewBox="0 0 512 512">
                                        <path fill="none" stroke="#ffffff" stroke-linecap="round" stroke-linejoin="round"
...
<script>
    $(document).ready(function() {
        $('.accordion-button').on('click', function() {
            ...
            $('.accordion-collapse').not(targetElement).collapse('hide');
